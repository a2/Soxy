...
{
	NSDictionary *requestHeaderFields = (__bridge_transfer NSDictionary *) CFHTTPMessageCopyAllHeaderFields(aRequest);
	NSURL *requestURL = (__bridge_transfer NSURL *) CFHTTPMessageCopyRequestURL(aRequest);
	NSString *method = (__bridge_transfer NSString *) CFHTTPMessageCopyRequestMethod(aRequest);
	...
	server:(SXHTTPServer *)aServer
{
	...
	if (self != nil)
	{
		...
		url = [requestURL copy];
		headerFields = [requestHeaderFields copy];
		fileHandle = requestFileHandle;
		server = aServer;
